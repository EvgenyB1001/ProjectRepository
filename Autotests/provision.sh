...
set_db_from_dump() {
	if [[ ! -f ${wordpress_database_dumped} ]]; then
		mysql -uroot --password=root wordpress < /vagrant/wordpress_dump.sql
	fi
}

configure_wordpress() {
    sed -e "s/define('DB_NAME'.*/define('DB_NAME', 'wordpress');/g" \
        -e "s/define('DB_USER'.*/define('DB_USER', 'wp_user');/g" \
        -e "s/define('DB_PASSWORD'.*/define('DB_PASSWORD', 'password');/g" \
        -e "s/define('WP_DEBUG'.*/define('WP_DEBUG', true);/g" \
