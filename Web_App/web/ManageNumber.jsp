...
<form action="<%=request.getContextPath()%>/" method="post">
    ...
    <table align="center" border="1" width="70%">
        ...
        %>
        <tr>
            <td colspan="2" align="left">Информация о телефоне владельца:
                <%=person.getSurname() + " " + person.getName() + " " + person.getMiddlename()%>
            </td>
        </tr>
        <tr>
            <td>Номер:</td>
            <%if (jsp_parameters.get("current_action").equals("editNumber")) {%>
            <td><input type="text" name="phone" value="<%=person.getPhones().get(numberId)%>"/></td>
            <%} else {%>
